...
class PlayingCardView: UIView {
    // setNeedsDisplay will call draw again aka redraw
    // setNeedsLayout will call layoutSubViews when you need show a new layout
    @IBInspectable
    ...
    private func configureCornerLabel(_ label: UILabel) {
        ...
        label.frame.size = CGSize.zero // expand in the both directions in sizeToFit
        label.sizeToFit()
    ...
    private func drawPips() {
        let pipsPerRowRank = [[0], [1], [1,1], [1,1,1], [2,2], [2,1,2], [2,2,2], [2,1,2,2], [2,2,2,2], [2,2,1,2,2], [2,2,2,2,2]]
        ...
        func createPipString(thatFits pipRect: CGRect) -> NSAttributedString {
            ...
            let maxHorizontalPipCount = CGFloat(pipsPerRowRank.reduce(0) { max($1.max() ?? 0, $0) })
        ...
        if pipsPerRowRank.indices.contains(rank) {
            let pipsPerRow = pipsPerRowRank[rank]
    ...
    override func draw(_ rect: CGRect) {
        ...
        roundedRect.addClip() // draw in this view
        ...
        if isFaceUp {
            ...
            if let cardBackImage = UIImage(named: "cardback", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
                cardBackImage.draw(in: bounds.insetBy(dx: cornerOffset, dy: cornerOffset) )
            }
        }
    }
